...
static void xhci_next_phys(iotxn_t* txn, xhci_transfer_state_t* state) {
    ...
    if (offset == 0) {
        state->page_index = 0;
        state->last_page_index = (length + align_adjust - 1) / PAGE_SIZE;
    ...
    if (txn->phys_count == 1) {
        ...
        if (remaining > XHCI_MAX_DATA_BUFFER) {
            state->next_length = XHCI_MAX_DATA_BUFFER;
        } else {
            state->next_length = remaining;
        }
        return;
    }
    ...
    mx_paddr_t phys = phys_addrs[state->page_index];
    state->next_phys = phys;
    mx_off_t packet_length = 0;
    bool discontiguous = false;
    uint64_t last_page_index = state->last_page_index;

    // loop through physical addresses looking for discontinuities
    while (remaining > 0 && packet_length < XHCI_MAX_DATA_BUFFER
           && ++state->page_index < last_page_index) {
        size_t increment = (PAGE_SIZE > remaining ? remaining : PAGE_SIZE);
        packet_length += increment;
        remaining -= increment;

        mx_paddr_t next = phys_addrs[state->page_index];
